...
def confirm_product():
    try:
        ...
        cursor.execute(
            "SELECT product_id, selling_price, product_quantity, cost_price FROM products WHERE product_name = ?",
            (product_name,))
        ...
        if quantity_bought > product["product_quantity"]:
            messagebox.showerror("Error",
                                 f"Not enough stock for '{product_name}'. Available: {product["product_quantity"]}")
            return
...
def refresh_customers_table():
    ...
    for customer in customers:
        ...
        cursor.execute(
            "SELECT sale_revenue, sale_date FROM sales WHERE customer_id = ? ORDER BY sale_date DESC LIMIT 1",
            (customer_id,))
...
def refresh_sales_table():
    ...
    cursor.execute(
        "SELECT sales.sale_id, customers.customer_contact, sales.sale_date, sales.sale_revenue, sales.sale_profit FROM sales LEFT JOIN customers ON sales.customer_id = customers.customer_id ORDER BY sales.sale_date DESC")
...
def search_product():
    ...
    for record in records:
        products_table.insert("", "end",
                              values=[record["product_id"], record["product_name"], record["product_quantity"],
...
def edit_product():
    ...
    def update_product():
        ...
        if messagebox.askyesno("Confirm Update", f"Are you sure you want to update Product ID {product_id}?"):
            try:
                cursor.execute(
                    "UPDATE products SET product_name = ?, selling_price = ?, cost_price = ? WHERE product_id = ?",
    ...
    ttk.Button(edit_window, text="Update Product", style="buttons.TButton", command=update_product).grid(row=4,
                                                                                                         column=1,
                                                                                                         padx=10,
                                                                                                         pady=10)
    ...
    def process_restock():
        ...
        if confirm:
            ...
            cursor.execute(
                ...
                (product_id, quantity_added, cost_price, restock_time))
            connection.commit()
...
def open_login_window():
    ...
    ttk.Label(login_window, text="Welcome to Retail Management System", font=("Inter", 18, "bold")).grid(row=0,
                                                                                                         column=0,
                                                                                                         columnspan=2,
                                                                                                         padx=10,
                                                                                                         pady=10)

    ttk.Label(login_window, text="Powered by:", font=("Inter", 16, "bold")).grid(row=1, column=0, padx=(40, 0),
                                                                                 pady=(30, 100))
    ...
    ttk.Button(login_window, text="Login", style="buttons.TButton", command=login).grid(row=4, column=1, padx=10,
                                                                                        pady=10)
...
ttk.Button(sale_input_frame, text="Confirm Product", style="buttons.TButton", command=confirm_product).grid(row=3,
                                                                                                            column=3,
                                                                                                            padx=5,
                                                                                                            pady=5)
